...
insert  into `origen`(`idorigen`,`origencol`) values ('P','PERUANA');
...
insert  into `reserva`(`Cod_Reserva`,`Hora`,`Fecha`,`Cant_Personas`,`Detalle`,`Cod_Cliente`,`Cod_Mesa`) values ('R','23:30:00','2017-11-22',1,'','1076225203','M-3');
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vtamesaresta` AS select `vtamesa`.`Cod_Mesa` AS `Cod_Mesa`,`vtamesa`.`Nombre` AS `Nombre`,`vtamesa`.`Num_Mesa` AS `Num_Mesa`,`vtamesa`.`Cant_Personas` AS `Cant_Personas`,`vtamesa`.`Fumador` AS `Fumador`,`vtamesa`.`Ubicacion` AS `Ubicacion`,`vtamesa`.`Estado` AS `Estado` from `vtamesa` where (`vtamesa`.`Nombre` = 'DON LUCHO') */;
